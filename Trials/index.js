...
function loadPandoc() {
  ...
  pandocReadyPromise = (async () => {
    ...
    const response = await fetch("./src/pandoc.wasm");
...
window.pandocApp = function () {
  return {
    ...
    get effectiveOutputFormat() {
      ...
      const outFile =
        this.outputFilename.trim() || this.outputFilenamePlaceholder;
    ...
    get outputFilenamePlaceholder() {
      if (this.outputFormat !== "auto") {
        const outExt =
          this.extensionByFormat[this.outputFormat] || this.outputFormat;
    ...
    get isHtmlFormat() {
      return this.htmlFormats.includes(this.effectiveOutputFormat);
    },
    get isMarkdownFormat() {
      return this.markdownFormats.includes(this.effectiveOutputFormat);
    },
    get isLatexLikeFormat() {
      return this.latexCitationFormats.includes(this.effectiveOutputFormat);
    },
    get supportsCaptionPosition() {
      return this.captionPositionFormats.includes(this.effectiveOutputFormat);
    },
    get supportsAscii() {
      return this.asciiFormats.includes(this.effectiveOutputFormat);
    },
    get supportsTopLevelDivision() {
      return this.topLevelDivisionFormats.includes(this.effectiveOutputFormat);
    },
    get supportsListOf() {
      return this.listOfFormats.includes(this.effectiveOutputFormat);
    },
    get isBinaryOutput() {
      return this.binaryFormats.includes(this.effectiveOutputFormat);
    },
    ...
    get showStripComments() {
      ...
      return (
        fmt === "auto" ||
        this.markdownFormats.includes(fmt) ||
        fmt === "textile"
      );
    },
    ...
    get showDefaultImageExtension() {
      const fmt = this.effectiveInputFormat;
      return (
        fmt === "auto" || this.markdownFormats.includes(fmt) || fmt === "latex"
      );
    },
    ...
    get showExtensionsTab() {
      return (
        (this.inputFormat !== "auto" && this.inputExtensionsList.length > 0) ||
    ...
    get showFormatTab() {
      const fmt = this.effectiveOutputFormat;
      return (
        this.showStripComments ||
        this.showDefaultImageExtension ||
        this.showAbbreviations ||
        this.isHtmlFormat ||
        this.isMarkdownFormat ||
        fmt === "rst" ||
        ["latex", "beamer"].includes(fmt) ||
        ["typst", "pdf"].includes(fmt) ||
        this.supportsCaptionPosition ||
        this.supportsAscii ||
        this.supportsTopLevelDivision ||
        this.supportsListOf ||
        this.docFormats.includes(fmt)
      );
    },
    ...
    get isEpubFormat() {
      return ["epub", "epub2", "epub3"].includes(this.effectiveOutputFormat);
    },
    get showChunkedTab() {
      return this.effectiveOutputFormat === "chunkedhtml";
    },
    get showIpynbTab() {
      return this.effectiveOutputFormat === "ipynb";
    },
    get showSlidesTab() {
      return this.slideFormats.includes(this.effectiveOutputFormat);
    },
    get showReferenceDoc() {
      return this.docFormats.includes(this.effectiveOutputFormat);
    },
    ...
    init() {
      ...
      console.log(
        "Pandoc converter initialized (pandoc.wasm loading in background)",
      );
    },
    ...
    handleBibFile(e) {
      if (e.target.files[0]) {
        this.bibFile = e.target.files[0];
        this.hasChanges = true;
      }
    },
    handleCslFile(e) {
      if (e.target.files[0]) {
        this.cslFile = e.target.files[0];
        this.cslStyleName = "";
        this.hasChanges = true;
      }
    },
    handleAbbrevFile(e) {
      if (e.target.files[0]) {
        this.abbrevFile = e.target.files[0];
        this.hasChanges = true;
      }
    },
    handleAbbreviationsFile(e) {
      if (e.target.files[0]) {
        this.abbreviationsFile = e.target.files[0];
        this.hasChanges = true;
      }
    },
    handleHighlightTheme(e) {
      if (e.target.files[0]) {
        this.highlightThemeFile = e.target.files[0];
        this.hasChanges = true;
      }
    },
    ...
    handleReferenceDoc(e) {
      if (e.target.files[0]) {
        this.referenceDoc = e.target.files[0];
        this.hasChanges = true;
      }
    },
    handleMetadataFile(e) {
      if (e.target.files[0]) {
    ...
    handleEpubCover(e) {
      if (e.target.files[0]) {
        this.epubCoverImage = e.target.files[0];
        this.hasChanges = true;
      }
    },
    handleEpubMetadata(e) {
      if (e.target.files[0]) {
        this.epubMetadataFile = e.target.files[0];
        this.hasChanges = true;
      }
    },
    ...
    handleCssFiles(e) {
      this.addFilesToList(e, this.cssFiles);
    },
    removeCssFile(name) {
      this.removeFileFromList(name, this.cssFiles);
    },
    handleSyntaxDefs(e) {
      this.addFilesToList(e, this.syntaxDefinitions);
    },
    removeSyntaxDef(name) {
      this.removeFileFromList(name, this.syntaxDefinitions);
    },
    handleEpubFonts(e) {
      this.addFilesToList(e, this.epubFonts);
    },
    removeEpubFont(name) {
      this.removeFileFromList(name, this.epubFonts);
    },
    handleHeaderFiles(e) {
      this.addFilesToList(e, this.headerFiles);
    },
    removeHeaderFile(name) {
      this.removeFileFromList(name, this.headerFiles);
    },
    handleBeforeBodyFiles(e) {
      this.addFilesToList(e, this.beforeBodyFiles);
    },
    removeBeforeBodyFile(name) {
      this.removeFileFromList(name, this.beforeBodyFiles);
    },
    handleAfterBodyFiles(e) {
      this.addFilesToList(e, this.afterBodyFiles);
    },
    removeAfterBodyFile(name) {
      this.removeFileFromList(name, this.afterBodyFiles);
    },
    handleLuaFilters(e) {
      this.addFilesToList(e, this.luaFilters);
    },
    removeLuaFilter(name) {
      this.removeFileFromList(name, this.luaFilters);
    },
    ...
    async loadCslStyles() {
      ...
      try {
        const response = await fetch(
          "https://data.jsdelivr.com/v1/package/gh/citation-style-language/styles@master/flat",
        );
        ...
        this.cslStylesList = data.files
          .map((f) => f.name)
          .filter(
            (name) => name.endsWith(".csl") && name.lastIndexOf("/") === 0,
          )
    ...
    async fetchCslStyle() {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`Style "${this.cslStyleName}" not found`);
        const cslContent = await response.text();
        const blob = new Blob([cslContent], { type: "application/xml" });
        this.cslFile = new File([blob], `${this.cslStyleName}.csl`, {
    ...
    async updateExtensions() {
      ...
      if (this.inputFormat !== "auto") {
        try {
          const extData = await window.pandocModule.query({
          ...
          console.warn("Could not get input extensions:", e);
        }
      }
      ...
      if (outFmt !== "auto") {
        try {
          ...
          console.warn("Could not get output extensions:", e);
        }
      }
    },
    ...
    getExtensionChecked(direction, ext) {
      const extState =
        direction === "input" ? this.inputExtensions : this.outputExtensions;
    ...
    buildOptions() {
      ...
      // Input format
      if (this.inputFormat !== "auto") {
        opts.from = this.buildFormatWithExtensions(
          this.inputFormat,
          this.inputExtensions,
        );
      ...
      // Output format
      if (this.outputFormat !== "auto") {
        opts.to = this.buildFormatWithExtensions(
          this.outputFormat,
          this.outputExtensions,
        );
      }
      ...
      if (this.opts.numberOffset.trim()) {
        opts["number-offset"] = this.opts.numberOffset
          .trim()
          .split(",")
          .map((n) => parseInt(n.trim()) || 0);
      }
      if (this.opts.resourcePath.trim())
        opts["resource-path"] = this.opts.resourcePath.trim();
      if (this.opts.stripComments) opts["strip-comments"] = true;
      if (this.opts.defaultImageExtension.trim())
        opts["default-image-extension"] =
          this.opts.defaultImageExtension.trim();
      if (this.abbreviationsFile)
        opts.abbreviations = this.abbreviationsFile.name;

      // Track changes
      if (this.opts.trackChanges)
        opts["track-changes"] = this.opts.trackChanges;
      ...
      if (this.htmlFormats.includes(baseOutFmt)) {
        ...
        if (this.opts.titlePrefix.trim())
          opts["title-prefix"] = this.opts.titlePrefix.trim();
        if (this.opts.emailObfuscation)
          opts["email-obfuscation"] = this.opts.emailObfuscation;
        if (this.opts.idPrefix.trim())
          opts["id-prefix"] = this.opts.idPrefix.trim();
      ...
      if (this.markdownFormats.includes(baseOutFmt)) {
        ...
        if (this.opts.referenceLocation)
          opts["reference-location"] = this.opts.referenceLocation;
        if (this.opts.markdownHeadings)
          opts["markdown-headings"] = this.opts.markdownHeadings;
      }
      ...
      if (this.captionPositionFormats.includes(baseOutFmt)) {
        if (this.opts.figureCaptionPosition)
          opts["figure-caption-position"] = this.opts.figureCaptionPosition;
        if (this.opts.tableCaptionPosition)
          opts["table-caption-position"] = this.opts.tableCaptionPosition;
      }

      // ASCII
      if (this.asciiFormats.includes(baseOutFmt)) {
        const asciiChecked = this.htmlFormats.includes(baseOutFmt)
          ? this.opts.asciiHtml
          : this.opts.ascii;
      ...
      // Top-level division
      if (
        this.topLevelDivisionFormats.includes(baseOutFmt) &&
        this.opts.topLevelDivision
      ) {
      ...
      if (["epub", "epub2", "epub3"].includes(baseOutFmt)) {
        ...
        if (this.epubMetadataFile)
          opts["epub-metadata"] = this.epubMetadataFile.name;
        ...
        if (this.opts.epubSubdirectory.trim())
          opts["epub-subdirectory"] = this.opts.epubSubdirectory.trim();
      }
      ...
      this.customVariables.forEach((v) => {
        if (v.key.trim() && v.value.trim())
          variables[v.key.trim()] = v.value.trim();
      });
      if (Object.keys(variables).length > 0) opts.variables = variables;

      // Chunked HTML
      if (baseOutFmt === "chunkedhtml") {
        if (this.opts.splitLevel)
          opts["split-level"] = parseInt(this.opts.splitLevel);
        if (this.opts.chunkTemplate.trim())
          opts["chunk-template"] = this.opts.chunkTemplate.trim();
      }
      ...
      if (this.slideFormats.includes(baseOutFmt)) {
        if (this.opts.slideLevel)
          opts["slide-level"] = parseInt(this.opts.slideLevel);
    ...
    async convert() {
      ...
      try {
        ...
        if (this.abbreviationsFile)
          files[this.abbreviationsFile.name] = this.abbreviationsFile;
        this.cssFiles.forEach((file) => {
          files[file.name] = file;
        });
        if (this.highlightThemeFile)
          files[this.highlightThemeFile.name] = this.highlightThemeFile;
        this.syntaxDefinitions.forEach((file) => {
          files[file.name] = file;
        });
        if (this.templateFile)
          files[this.templateFile.name] = this.templateFile;
        if (this.referenceDoc)
          files[this.referenceDoc.name] = this.referenceDoc;
        if (this.metadataFile)
          files[this.metadataFile.name] = this.metadataFile;
        if (this.epubCoverImage)
          files[this.epubCoverImage.name] = this.epubCoverImage;
        if (this.epubMetadataFile)
          files[this.epubMetadataFile.name] = this.epubMetadataFile;
        this.epubFonts.forEach((font) => {
          files[font.name] = font;
        });
        this.headerFiles.forEach((file) => {
          files[file.name] = file;
        });
        this.beforeBodyFiles.forEach((file) => {
          files[file.name] = file;
        });
        this.afterBodyFiles.forEach((file) => {
          files[file.name] = file;
        });
        this.luaFilters.forEach((file) => {
          files[file.name] = file;
        });
        ...
        if (isPdfTypst) {
          ...
          typstOptions.to = this.buildFormatWithExtensions(
            "typst",
            this.outputExtensions,
          );
          ...
          const typstResult = await window.pandocModule.convert(
            typstOptions,
            stdin,
            files,
          );
    ...
    displayResults(result, options, files) {
      ...
      if (result.warnings && result.warnings.length > 0) {
        result.warnings.forEach((w) => {
          ...
          this.messages.push({
            type: msgType,
            text: w.pretty || w.message || JSON.stringify(w),
          });
        });
      }
